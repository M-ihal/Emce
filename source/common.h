...
#define ZERO_ARRAY(arr) memset(arr, 0, sizeof(arr)) // Static array, memset required
...
#define GL_CHECK(...) glGetError(); __VA_ARGS__; { auto COMBINE(error_code, __LINE__) = glGetError(); if(COMBINE(error_code, __LINE__) != GL_NO_ERROR) { log_error("line:%d code:%d str:%s exp:%s\n", __LINE__, COMBINE(error_code, __LINE__), glewGetErrorString(COMBINE(error_code, __LINE__)), #__VA_ARGS__); } }
#else
